...
mcp = FastMCP("NightEye", description="NightEye DFIR AI Agent Integration")


def start_server(port: int = 4509) -> None:
    ...
    logger.info("Starting NightEye MCP Server on port %d for case %s", port, case_dir.name)
    ...
    # Run the server (using stdio or HTTP depending on FastMCP version)
    # The requirement is streamable HTTP or stdio. For local Claude Code we use stdio usually.
    # FastMCP's run() method handles stdio by default or SSE if configured.
    # For HTTP server, we can use uvicorn or the built-in fastmcp run.
